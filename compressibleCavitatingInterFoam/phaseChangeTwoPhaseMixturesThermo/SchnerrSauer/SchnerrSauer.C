...
namespace phaseChangeTwoPhaseMixturesThermo
{
    ...
    (
        phaseChangeTwoPhaseMixtureThermo,
        SchnerrSauer,
        components
    );
}
...
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::SchnerrSauer
(
...
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::rRb
(
...
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::alphaNuc() const
{
...
Foam::tmp<Foam::volScalarField>
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::pCoeff
(
...
) const
{
    ...
    volScalarField rho
    (
        limitedAlpha1*thermo1().rho() + (scalar(1) - limitedAlpha1)*thermo2().rho()
    );

    return
        (3*thermo1().rho()*thermo2().rho())*sqrt(2/(3*thermo1().rho()))
       *rRb(limitedAlpha1)/(rho*sqrt(mag(p - pSat()) + 0.01*pSat()));
}


Foam::Pair<Foam::tmp<Foam::volScalarField>>
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::mDotAlphal() const
{
...
Foam::Pair<Foam::tmp<Foam::volScalarField>>
Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::mDotP() const
{
    const volScalarField& p = alpha1().db().lookupObject<volScalarField>("p");
    volScalarField pCoeff(this->pCoeff(p));
...
void Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::correct()
{}


bool Foam::phaseChangeTwoPhaseMixturesThermo::SchnerrSauer::read()
{
